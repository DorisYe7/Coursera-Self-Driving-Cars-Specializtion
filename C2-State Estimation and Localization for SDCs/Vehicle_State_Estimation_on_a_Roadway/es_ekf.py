...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    Kk = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + sensor_var))
    ...
    q_hat = Quaternion(euler=dxk[6:]).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_check = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2)/2*(Cns.dot(imu_f.data[k-1])+g)
    v_check = v_est[k-1] + delta_t*(Cns.dot(imu_f.data[k-1])+g)
    ...
    Fk = np.zeros((9,9))
    Fk[0:3][:] = np.concatenate((np.eye(3),np.eye(3)*delta_t,np.zeros((3,3))),axis=1)
    Fk[3:6][:] = np.concatenate((np.zeros((3,3)),np.eye(3),-(Cns@skew_symmetric(imu_f.data[k-1]))*delta_t),axis=1)
    Fk[6:][:] = np.concatenate((np.zeros((3,3)),np.zeros((3,3)),np.eye(3)),axis=1)
    
    # Motion model jacobian wrt noise
    acc_var = var_imu_f**2
    gyro_var = var_imu_w**2
    Qk = np.diag([acc_var,acc_var,acc_var,gyro_var,gyro_var,gyro_var])*delta_t**2
    
    # 2. Propagate uncertainty
    p_cov_check = Fk.dot(p_cov[k-1]).dot(Fk.T) + l_jac.dot(Qk).dot(l_jac.T)
    ...
    if np.where(lidar.t == imu_f.t[k])[0].shape[0] != 0:
        index_lidar = np.where(lidar.t == imu_f.t[k])[0][0]
    if np.where(gnss.t == imu_f.t[k])[0].shape[0] != 0:
        index_gnss = np.where(gnss.t == imu_f.t[k])[0][0]
    
    if index_lidar != 0 and index_gnss == 0:
        sensor_var = np.eye(3)*var_lidar
        y_k = lidar.data[index_lidar]
        p_est[k],v_est[k],q_est[k],p_cov[k] = measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check)
        lidar_i += 1
    elif index_lidar == 0 and index_gnss != 0:
        sensor_var = np.eye(3)*var_gnss
        y_k = gnss.data[index_gnss]
        p_est[k],v_est[k],q_est[k],p_cov[k] = measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check)
        gnss_i += 1
    elif index_lidar != 0 and index_gnss != 0:
        sensor_var = np.eye(3)*var_gnss
